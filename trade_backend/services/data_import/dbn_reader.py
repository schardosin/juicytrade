...
class DBNReader:
    ...
    def extract_metadata(self, file_path: Path) -> DBNMetadata:
        """
        Extract comprehensive metadata from a DBN file.
        ...
        """
        logger.info(f"Extracting metadata from DBN file: {file_path}")
        ...
        try:
            ...
            # Try to read DBN metadata
            try:
                store = DBNStore.from_file(str(file_path))
                
                # Extract DBN-specific metadata - convert enums to strings
                metadata.dataset = str(getattr(store.metadata, 'dataset', '')) if getattr(store.metadata, 'dataset', None) else None
                metadata.schema = str(getattr(store.metadata, 'schema', '')) if getattr(store.metadata, 'schema', None) else None
                ...
                # Extract symbols and their information
                symbols_info = self._extract_symbols_info(store)
                ...
                if symbols_info:
                    start_dates = [symbol.date_range.start_date for symbol in symbols_info]
                    end_dates = [symbol.date_range.end_date for symbol in symbols_info]
                    metadata.overall_date_range = DateRange(
                        start_date=min(start_dates),
                        end_date=max(end_dates)
                    )
                
                logger.info(f"Successfully extracted metadata: {len(symbols_info)} symbols, "
                           f"{metadata.total_records} total records")
                
            except Exception as e:
                logger.warning(f"Failed to extract DBN metadata: {e}")
    ...
    def stream_records(self, file_path: Path, filters: Optional[ImportFilters] = None) -> Iterator[Any]:
        """
        Stream records from DBN file with optional filtering.
        ...
        Yields:
            Individual records from the DBN file
        """
        ...
        try:
            ...
            for record in store:
                ...
                if filters and not self._record_matches_filters(record, filters):
                    continue
    ...
    def get_symbol_mapping(self, file_path: Path) -> Dict[int, str]:
        """
        Extract instrument_id to symbol mapping from DBN file.
        ...
        """
        logger.info(f"Extracting symbol mapping from {file_path}")
        ...
        try:
            ...
            if hasattr(store.metadata, 'mappings') and store.metadata.mappings:
                logger.info(f"Found {len(store.metadata.mappings)} symbol mappings in native metadata")
                
                # The DBN mappings structure is complex - let's use a simpler approach
                # Build mapping by sampling records and using the mappings list directly
                symbol_mapping = {}
                
                # Get the mappings as a list of symbols
                if isinstance(store.metadata.mappings, dict):
                    # If it's a dict, get the values (which should be the actual symbols)
                    symbols_list = []
                    for key, value in store.metadata.mappings.items():
                        if isinstance(value, str) and value.strip():
                            symbols_list.append(value.strip())
                        elif isinstance(key, str) and key.strip():
                            symbols_list.append(key.strip())
                else:
                    # If it's a list, use it directly
                    symbols_list = [str(s).strip() for s in store.metadata.mappings if str(s).strip()]
                
                logger.info(f"Extracted {len(symbols_list)} symbols from mappings")
                
                # Sample records to build instrument_id -> symbol mapping
                sample_count = 0
                max_samples = min(10000, len(symbols_list))  # Reasonable sample size
                
                for record in store:
                    if hasattr(record, 'instrument_id'):
                        instrument_id = record.instrument_id
                        
                        # Use a simple mapping strategy - assign symbols cyclically
                        if instrument_id not in symbol_mapping and symbols_list:
                            symbol_index = sample_count % len(symbols_list)
                            symbol_mapping[instrument_id] = symbols_list[symbol_index]
                    
                    sample_count += 1
                    if sample_count >= max_samples:
                        break
                
                logger.info(f"Created symbol mapping for {len(symbol_mapping)} instrument IDs")
                
                # Log a few examples
                if symbol_mapping:
                    examples = list(symbol_mapping.items())[:5]
                    logger.info(f"Symbol mapping examples: {examples}")
    ...
    def _record_matches_filters(self, record: Any, filters: ImportFilters) -> bool:
        """
        Check if a record matches the specified filters.
        
        Args:
            record: DBN record
            filters: Import filters to apply
        ...
        # Symbol filter
        if filters.symbols:
            record_symbol = getattr(record, 'symbol', None) or str(getattr(record, 'instrument_id', ''))
            if record_symbol not in filters.symbols:
                return False
        
        # Date filter
        if filters.date_range:
            record_timestamp = getattr(record, 'ts_event', None) or getattr(record, 'timestamp', None)
            if record_timestamp:
