...
from src.models.strategy_state import StrategyState, StateValidationLevel
...
class BaseStrategy(ABC):
    ...
    async def get_market_data(self) -> Dict[str, Any]:
        ...
        if self.data_provider:
            ...
            # Case 1: Provider has get_all_market_data (e.g. BacktestEngine)
            if hasattr(self.data_provider, 'get_all_market_data'):
                if asyncio.iscoroutinefunction(self.data_provider.get_all_market_data):
                    return await self.data_provider.get_all_market_data()
                return self.data_provider.get_all_market_data()
                
            # Case 2: Provider has get_market_data(symbol)
            if hasattr(self.data_provider, 'get_market_data'):
                symbols = self.get_all_required_symbols()
                for symbol in symbols:
                    try:
                        if asyncio.iscoroutinefunction(self.data_provider.get_market_data):
