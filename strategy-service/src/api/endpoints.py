...
from src.persistence.strategy_registry import StrategyRegistry
from src.persistence.strategy_validator import StrategyValidator
from src.execution.execution_engine import StrategyExecutionEngine
...
# ============================================================================
# Strategy Service Status Endpoint
# ============================================================================

@router.get("/status", response_model=Dict[str, Any])
async def get_strategy_service_status():
    """
    Check if the strategy service is operational.
    
    This endpoint is used by the Go backend to determine if the
    strategies module is running and should be shown in the UI.
    """
    try:
        # Check database connectivity
        from src.persistence.database import strategy_db_manager
        db_healthy = strategy_db_manager.health_check().get("database_connected", False)
        
        # Get strategy count
        registry = get_strategy_registry()
        strategies = registry.store.get_all_strategies()
        ...
        if db_healthy:
            return {
                ...
                "message": "Strategy service is operational"
            }
        else:
            return {
                "status": "degraded",
                ...
                "message": "Strategy service is running but database is not connected"
            }
            
    except Exception as e:
        logger.error(f"Error checking strategy service status: {e}")
        return {
            "status": "error",
            "strategies_count": 0,
            "database_connected": False,
            "message": f"Strategy service error: {str(e)}"
        }
...
async def get_strategy_parameters(strategy_id: str):
    ...
    try:
        ...
        from src.data.mock_providers import MockDataProvider, MockOrderExecutor
...
async def create_strategy_configuration(strategy_id: str, config_data: Dict[str, Any]):
    ...
    try:
        from src.persistence.database import strategy_db_manager
        from src.persistence.models import StrategyConfiguration
        import uuid
...
async def run_strategy_backtest(strategy_id: str, backtest_request: Dict[str, Any]):
    ...
    try:
        from src.persistence.database import strategy_db_manager
        from src.persistence.models import BacktestRun
        from datetime import datetime
        import uuid
        ...
        with strategy_db_manager.get_session() as session:
            ...
            try:
                ...
                try:
                    from src.data.aggregation_service import get_aggregation_service
                    from src.data.cbbo_service import get_cbbo_service
                    
                    aggregation_service = get_aggregation_service()
                    cbbo_service = get_cbbo_service()
                    
                    logger.info(f"✅ Using DataAggregationService and CBBOService for backtest (imported parquet data)")
                    logger.info(f"   Aggregation service: {aggregation_service}")
                    logger.info(f"   CBBO service: {cbbo_service}")
                except Exception as e:
                    logger.error(f"❌ BACKTEST FAILED: Could not import/initialize data services: {e}", exc_info=True)
                    return {
                        "success": False,
...
async def get_backtest_runs(strategy_id: Optional[str] = None):
    ...
    try:
        from src.persistence.database import strategy_db_manager
        from src.persistence.models import BacktestRun, Strategy
...
async def start_strategy_with_parameters(strategy_id: str, start_request: Dict[str, Any], request: Request):
    ...
    try:
        from src.persistence.database import strategy_db_manager
        from src.persistence.models import StrategyExecution
        from datetime import datetime
        import uuid
