...
export function useOrderManagement(options = {}) {
  ...
  const buildButterflyOrderData = (params) => {
    ...
    if (strategyType === "IRON Condor") {
      const longPut = findOption(butterflyInfo.long_put_strike, "put");
      const shortPut = findOption(butterflyInfo.short_put_strike, "put");
      const shortCall = findOption(butterflyInfo.short_call_strike, "call");
      const longCall = findOption(butterflyInfo.long_call_strike, "call");
      ...
      legs = [
        {
          symbol:
            ...
            `${symbol}_${formatExpiry(expiry)}_${
              butterflyInfo.long_put_strike
            }_P`,
          side: "buy",
          ratio_qty: "1",
          type: "Put",
          strike: `$${butterflyInfo.long_put_strike.toFixed(2)}`,
          price: butterflyInfo.long_put_price,
        ...
        },
        {
          symbol:
            shortPut?.symbol ||
            `${symbol}_${formatExpiry(expiry)}_${
              butterflyInfo.short_put_strike
            }_P`,
          side: "sell",
          ratio_qty: "1",
          type: "Put",
          strike: `$${butterflyInfo.short_put_strike.toFixed(2)}`,
          price: butterflyInfo.short_put_price,
          displaySymbol: symbol,
          date: formatExpiry(expiry),
        },
        {
          symbol:
            shortCall?.symbol ||
            `${symbol}_${formatExpiry(expiry)}_${
              butterflyInfo.short_call_strike
            }_C`,
          side: "sell",
          ratio_qty: "1",
          type: "Call",
          strike: `$${butterflyInfo.short_call_strike.toFixed(2)}`,
          price: butterflyInfo.short_call_price,
          displaySymbol: symbol,
          date: formatExpiry(expiry),
        },
        {
          symbol:
            longCall?.symbol ||
            `${symbol}_${formatExpiry(expiry)}_${
              butterflyInfo.long_call_strike
            }_C`,
          side: "buy",
          ratio_qty: "1",
          type: "Call",
          strike: `$${butterflyInfo.long_call_strike.toFixed(2)}`,
          price: butterflyInfo.long_call_price,
          displaySymbol: symbol,
          date: formatExpiry(expiry),
        },
      ];
